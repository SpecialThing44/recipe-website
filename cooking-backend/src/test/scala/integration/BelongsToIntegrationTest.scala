...
import domain.ingredients.{Quantity, Unit => IngUnit}
...
class BelongsToIntegrationTest extends IntegrationTestFramework {
  ...
  def recipeInputWith(ingredients: Seq[Ingredient], public: Boolean, name: String): RecipeInput = {
  ...
  it should "list ingredients that belong to a user when filtering by belongsToUser" in {
    val i1 = tomato
    val i2 = createTestIngredient(IngredientInput(
      name = "Onion",
      aliases = Seq("onion"),
    ...
    val resultsForA = listIngredients(Filters.empty().copy(belongsToUser = Some(user1.id)))
    ...
    val resultsForB = listIngredients(Filters.empty().copy(belongsToUser = Some(user2.id)))
  ...
  it should "list only own or public recipes when filtering by belongsToUser and exclude private recipes of others" in {
    val recipePublic1 = createTestRecipe(recipeInputWith(Seq(tomato), public = true, name = "A Public"))
    ...
    login(user1.id)
    val user1Recipes = listRecipes(Filters.empty().copy(belongsToUser = Some(user1.id)))
    user1Recipes.map(_.id).toSet shouldBe Set(recipePublic1.id, recipePrivate1.id)
    login(user2.id)
    val user2Recipes = listRecipes(Filters.empty().copy(belongsToUser = Some(user2.id)))
    user2Recipes.map(_.id).toSet shouldBe Set(recipePublic2.id, recipePrivate2.id)
