...
class UsersAnalyticalIntegrationTest extends IntegrationTestFramework {
  ...
  it should "rank users by ingredient similarity and apply minScore, excluding the analyzed user" in {
    val target =
      createTestAdminUser(UserInput("Target User", "target@example.com"))
    val candidateA =
      createTestAdminUser(UserInput("Candidate A", "candA@example.com"))
    val candidateB =
      createTestAdminUser(UserInput("Candidate B", "candB@example.com"))
    val candidateD =
      createTestAdminUser(UserInput("Candidate D", "candD@example.com"))
    login(target.id)
    ...
    createTestRecipe(
      recipeInput("T-Recipe", Seq("common", "ta"), Seq(tomato, onion))
    )

    login(candidateA.id)
    createTestRecipe(
      recipeInput("A-Recipe", Seq("common", "ta"), Seq(tomato, onion))
    )
  ...
  it should "rank users by tag similarity" in {
    val target =
      createTestAdminUser(UserInput("Tag Target", "tagtarget@example.com"))
    val candidateA =
      createTestAdminUser(UserInput("Tag Candidate A", "tagcanda@example.com"))
    val candidateB =
      createTestAdminUser(UserInput("Tag Candidate B", "tagcandb@example.com"))
    login(target.id)
    val tomato = createTestIngredient(ingredientInputTomato)
    val onion = createTestIngredient(ingredientInputOnion)

    createTestRecipe(
      recipeInput("TT-1", Seq("common", "ta"), Seq(tomato, onion))
    )

    login(candidateA.id)
    createTestRecipe(
      recipeInput("TA-1", Seq("common", "ta"), Seq(tomato, onion))
    )
  ...
  it should "rank users by co-save similarity and respect minScore" in {
    val target =
      createTestAdminUser(UserInput("Save Target", "savetarget@example.com"))
    val candidateA = createTestAdminUser(
      UserInput("Save Candidate A", "savecanda@example.com")
    )
    val candidateB = createTestAdminUser(
      UserInput("Save Candidate B", "savecandb@example.com")
    )
    login(target.id)
    ...
    val filtersLoose = filtersStrict.copy(coSaveSimilarity =
      Some(
        SimilarityFilter(alpha = 1.0, beta = 0.0, gamma = 0.0, minScore = 0.4)
      )
    )
  ...
  it should "rank users by combined ingredient, tag, and co-save similarity" in {
    val target =
      createTestAdminUser(UserInput("Combo Target", "combotarget@example.com"))
    val candidateA = createTestAdminUser(
      UserInput("Combo Candidate A", "combocanda@example.com")
    )
    val candidateB = createTestAdminUser(
      UserInput("Combo Candidate B", "combocandb@example.com")
    )
    login(target.id)
    val tomato = createTestIngredient(ingredientInputTomato)
    val onion = createTestIngredient(ingredientInputOnion)
    ...
    login(candidateA.id)
    val tRecipe = createTestRecipe(
      recipeInput("C-T", Seq("common", "ta"), Seq(tomato, onion))
    )
    val aRecipe = createTestRecipe(
      recipeInput("C-A", Seq("common", "ta"), Seq(tomato, onion))
    )

    login(candidateB.id)
    val bRecipe =
      createTestRecipe(recipeInput("C-B", Seq("common"), Seq(tomato)))
    ...
    val filters = Filters
      ...
      .copy(
        ...
        coSaveSimilarity = Some(
          SimilarityFilter(alpha = 1.0, beta = 0.0, gamma = 0.0, minScore = 0.0)
        ),
        tagSimilarity = Some(
          SimilarityFilter(alpha = 0.0, beta = 0.0, gamma = 1.0, minScore = 0.0)
        )
      )
