...
import domain.users.{AvatarUrls, User}
...
object UserConverter extends Converter[User] {
  ...
  override def toDomain(record: util.Map[String, AnyRef]): User = {
    val thumbnail =
      Option(record.get(avatarThumbnailUrlField).toString).filter(_.nonEmpty)
    val medium =
      Option(record.get(avatarMediumUrlField).toString).filter(_.nonEmpty)
    ...
    User(
      ...
      identity =
        Option(record.get(identityField)).map(_.toString).getOrElse(""),
      countryOfOrigin =
