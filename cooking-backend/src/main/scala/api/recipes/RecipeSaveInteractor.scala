...
class RecipeSaveInteractor @Inject() (
  ...
  def save(recipeId: UUID): ZIO[ApiContext, Throwable, Recipe] = {
    for {
      ...
      _ <- if (recipe.createdBy.id == user.id) ZIO.fail(InputError("Users cannot save their own recipes")) else ZIO.unit
      _ <- if (!recipe.public) ZIO.fail(InputError("Users cannot save private recipes")) else ZIO.unit
