      ...
      ZIO.attempt {
        if (apiToken.isEmpty) {
          throw new RuntimeException(
            "Authentik API token is missing. Please set AUTH_API_TOKEN environment variable."
          )
        }
        ...
        if (!searchResponse.code.isSuccess) {
          throw new RuntimeException(
            s"Failed to search user in Authentik: ${searchResponse.code} ${searchResponse.body}"
          )
        }

        val pk = parse(searchResponse.body.getOrElse("{}"))
          .flatMap(
            _.hcursor.downField("results").downArray.downField("pk").as[Int]
          ) match {
          case Right(id) => id
          case Left(_)   =>
            ...
            parse(searchResponse.body.getOrElse("{}"))
              ...
              case Left(e) =>
                throw new RuntimeException(
                  s"User not found or PK invalid for username: $currentUsername. Error: $e"
                )
            }
        }
        ...
        if (bodyMap.nonEmpty) {
          ...
          if (!response.code.isSuccess) {
            throw new RuntimeException(
              s"Failed to update user in Authentik: ${response.code} ${response.body}"
            )
          }
        }
      }
